...
- Rendered graph instances found: 53
- Passed after fixes: 53
- Need human review: 0
...
| 52 | PASS | v30 | Advanced sensitivity | Fresnel-zone coherent look count | v30-live-fresnel-look-count | docs/model.js + docs/data/v30-results.js | window.V30_LIVE_MODEL.compute(defaults, window.V30_RESULTS).charts | Along-track position in kilometers. | Coherent look count. | HF effective coherent looks; VHF effective coherent looks; Aperture cap in samples | km, count | Effective looks are computed from first-Fresnel-zone diameter, limited by aperture samples, then reduced by phase-decorrelation loss. | The chart explains the coherent-gain proxy by showing the sample-count driver behind it. | It renders 3 series (HF effective coherent looks; VHF effective coherent looks; Aperture cap in samples) against Along-track position (km) and Coherent looks (count). Y-values range from 1 to 5 count. | None. |
| 53 | PASS | v30 | Advanced sensitivity | Windowed chirp response: frequency washout | v30-live-windowed-chirp-washout | docs/model.js + docs/data/v30-results.js | window.V30_LIVE_MODEL.compute(defaults, window.V30_RESULTS).charts | Categorical radar band. | Relative response in dB. | Center-frequency reflectivity offset; Windowed chirp-average reflectivity; Windowed response plus pulse gain; Window washout delta | dB | Frequency response is sampled across each chirp band in linear power, Hamming-weighted, averaged, converted to dB, and optionally summed with pulse gain. | The chart separates center-frequency assumptions from the band-averaged response actually used by the v30 sensitivity model. | It renders 4 series (Center-frequency reflectivity offset; Windowed chirp-average reflectivity; Windowed response plus pulse gain; Window washout delta) against Radar band and dB. Y-values range from -5.474 to 16.195 dB. | None. |
...
- Status by rendered instance: v30 PASS
- Research question: How many coherent along-track samples are available inside the first Fresnel-zone limit?
- Input data: Modeled depth, radar frequency, along-track spacing, coherence aperture cap, and phase-decorrelation setting.
...
- Axis meaning: x = Along-track position in kilometers.; y = Coherent look count.
- Expected behavior: Counts should be finite and non-negative; HF should generally allow more looks than VHF for the same depth and spacing.
- Interpretation: The chart explains the coherent-gain proxy by showing the sample-count driver behind it.
- What would make it misleading: Misleading if described as an actual processed aperture product or if counts become negative.
- Audit result: No issues after fixes.
...
- Status by rendered instance: v30 PASS
- Research question: Does a frequency-dependent response survive chirp window averaging, or does it mostly wash out?
- Input data: HF/VHF center frequency, bandwidth, frequency slope, Hamming window weights, pulse length, and window loss.
...
- Axis meaning: x = Categorical radar band.; y = Relative response in dB.
- Expected behavior: The chart should be a categorical bar chart in dB with finite values for both HF and VHF bands.
...
- What would make it misleading: Misleading if the band-averaged result is presented as a measured chirp processor or if dB values are averaged directly instead of averaging linear power first.
- Audit result: No issues after fixes.
