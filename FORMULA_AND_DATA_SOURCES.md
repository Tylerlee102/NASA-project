...
| Target-delay clutter geometry | `R = altitude + n*targetDepth`; `offset = sqrt(R^2 - altitude^2)`; `theta = asin(offset/R)`. | Altitude, target depth, ice refractive index. | D5, `targetDelayClutterGeometry`, `computeBreakdownPoint`, `computeRows`. | Flat-surface PRF/folding geometry: the main clutter angle is the off-nadir surface ring with the same delay as the selected nadir target. |
| PRF breakdown stress test | `maxDoppler = 2*speed*sin(theta)/wavelength`; `aliasThresholdPrf = 2*maxDoppler`; `requiredPrf = aliasThresholdPrf*safetyFactor`; `margin = effectivePrf/requiredPrf`; `f_alias = ((maxDoppler + PRF/2) mod PRF) - PRF/2`. | Flyby speed, wavelength, flat target-delay clutter angle, safety factor, usable PRF. | D5, `computeBreakdownPoint`, `buildCharts`. | Simplified flat-surface Doppler-Nyquist stress test, not a mission PRF schedule. It separates aliasing onset from exact zero-Doppler/nadir-bin folding. |
| Pulse/listen PRF ceiling | `pulseLimitedPrf = 1/(pulseLength + guard + receiverDeadTime + 2*(altitude + n*targetDepth)/c)`. | Pulse length, listen guard, receiver dead time, altitude, target depth, ice refractive index. | D5, `timingBudget`, `pulseLimitedPrf`. | Timing-budget proxy for how long the radar must wait before the next pulse. |
| Schedule-adjusted usable PRF | `effectivePrf = min(maxUsablePrf, pulseLimitedPrf) * clamp(cpiMs/max(prfUpdateIntervalMs,cpiMs),0.25,1)`. | Max usable PRF, pulse/listen ceiling, PRF update interval, CPI. | D5, `computeBreakdownPoint`. | Educational dwell/CPI guard; failure means the required PRF exceeds the usable window or schedule adaptation is too slow for the dwell. |
| Dirty-ice folded-clutter severity | `surfaceClutterDb = surfaceScatteringDb + dirtyIceClutterBoostDb`; folded clutter is reduced by the folding/risk proxy before comparison with target signal. | Surface scattering, dirty-ice boost, target signal, fold-depth overlap, PRF deficit. | D5, `effectiveSurfaceScatteringDb`, `computeRows`. | Dirty/rough ice affects how strong aliased clutter looks, not the Doppler Nyquist requirement itself. |
